...
function API.GetDataStore(_, name, scope)
	...
	function d.UpdateAsync(_, k, func)
		local v = func(data[k])
		assert(v ~= nil, "Value cannot be nil")
		data[k] = v
		if (updateListeners[k]) then
	...
	function d.GetAsync(_, k)
		return data[k]
	end
	function d.RemoveAsync(_, k)
		data[k] = nil
		if (updateListeners[k]) then
			for _,f in ipairs(updateListeners[k]) do
	...
	function d.OnUpdate(_, k, onUpdateFunc)
...
function API:GetOrderedDataStore(name, scope)
	...
	function d.IncrementAsync(_, k, delta)
	...
	function d.GetSortedAsync(_, isAscending, pageSize, minValue, maxValue)
...
function API.GetRequestBudgetForRequestType(_, requestType)
